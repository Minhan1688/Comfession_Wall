...
import type { UploadProps } from 'element-plus'
        ...
        const MyUserType = ref(Number(localStorage.getItem("user_type"))==2 ? "管理员" : "学生");
        ...
        function getReport(){
          axios.get("/api/student/report-post", { params: { "user_id": Number(localStorage.getItem("user_id")) } }).then((res) => {
            if (res.data.code === 200) {
        ...
        function reportPost (index) {
            ...
            if (newContent !== null) {
              if (newContent.trim() === '') return;
              axios.post("/api/student/report-post", {
                  "user_id": Number(localStorage.getItem("user_id")),
                  "post_id": posts.value[index].id,
                  "reason": newContent,
              }).then((res)=>{
...
<template>
    ...
    <ul class="nav-links">
      ...
      <li>
        ...
        <ul class="sub-menu">
          ...
          <a @click="()=>{showing=2;getReport();}">审批结果</a>
        </ul>
      </li>
    </ul>
  </div>
  ...
  <div class="container" v-if="showing===2">
            <div class="posts">
                <div v-for="report in reports" class="post">
                    <span class="lineInPost">编号：{{ report.post_id }}</span>
                    <br>
                    <span class="lineInPost">内容：{{ report.content }}</span>
                    <br>
                    <span class="lineInPost">理由：{{ report.reason }}</span>
                    <br>
                    <span v-show="report.status===0" class="lineInPost" style="color:white">状态：未审批</span>
                    <span v-show="report.status===1" class="lineInPost" style="color:green">状态：已通过</span>
                    <span v-show="report.status===2" class="lineInPost" style="color:red">状态：不通过</span>
                    <br>
                    <br>
                    <br>
                    <br>
